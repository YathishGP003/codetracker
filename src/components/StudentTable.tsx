...
import { Search, Eye, Trash2, Plus, Filter, Mail, AlertCircle, Users, ExternalLink, Download, Edit } from 'lucide-react';
...
const StudentTable: React.FC<StudentTableProps> = ({ students: propStudents }) => {
  ...
  const addTopCodeforcesUsers = async () => {
    const topUsers = [
      { name: 'Gennady Korotkevich', email: 'tourist@example.com', handle: 'tourist' },
      { name: 'Jiangly', email: 'jiangly@example.com', handle: 'jiangly' },
      { name: 'Orz Devin Wang', email: 'orzdevinwang@example.com', handle: 'orzdevinwang' },
      { name: 'Kevin114514', email: 'kevin114514@example.com', handle: 'Kevin114514' },
      { name: 'Radewoosh', email: 'radewoosh@example.com', handle: 'Radewoosh' },
      ...
      { name: 'Andrew He', email: 'ecnerwala@example.com', handle: 'ecnerwala' },
  ...
  const downloadCSV = () => {
    const headers = ['Name', 'Email', 'Phone Number', 'Codeforces Handle', 'Current Rating', 'Max Rating', 'Status', 'Last Updated'];
    const csvContent = [
      headers.join(','),
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8 space-y-4 lg:space-y-0">
          ...
          <div className="flex items-center space-x-3 flex-wrap">
            ...
            <div className="relative">
              ...
              {showFilterDropdown && (
                <div className={`absolute right-0 mt-2 w-48 rounded-2xl shadow-lg border z-50 ${
                  isDarkMode 
        ...
        {activeFilter !== 'all' && (
          <div className="mb-4 flex items-center justify-between">
            <span className={`text-sm ${isDarkMode ? 'text-slate-400' : 'text-gray-600'}`}>
              ...
              {activeFilter !== 'all' && ` (${filterOptions.find(opt => opt.value === activeFilter)?.label})`}
            </span>
            <button
        ...
        {students.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className={`mb-6 ${isDarkMode ? 'text-slate-400' : 'text-gray-600'}`}>
              Get started by adding your first student or adding top Codeforces users
            </p>
            ...
            <table className="w-full">
              <thead>
                <tr className={`border-b ${
                  ...
                  {['Name', 'CF Handle', 'Current Rating', 'Max Rating', 'Status', 'Actions'].map((header) => (
                    <th key={header} className={`text-left py-4 px-4 font-semibold text-sm ${
              ...
              <tbody>
                {filteredStudents.map((student) => (
                  ...
                  >
                    ...
                    <td className={`py-4 px-4`}>
                      <div className="flex flex-col">
                        <span className={`font-bold text-lg ${getRatingColor(student.maxRating)}`}>
                          {student.maxRating || 0}
                        </span>
                        <span className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>
