...
async function fetchCodeforcesUser(handle: string): Promise<CodeforcesUser | null> {
  try {
    const data = await makeCodeforcesRequest(`https://codeforces.com/api/user.info?handles=${handle}`);
...
async function fetchCodeforcesSubmissions(handle: string, from?: number, count?: number): Promise<CodeforcesSubmission[]> {
  try {
...
async function fetchCodeforcesRatingHistory(handle: string): Promise<CodeforcesRatingChange[]> {
  try {
    const data = await makeCodeforcesRequest(`https://codeforces.com/api/user.rating?handle=${handle}`);
    return data.result || [];
  } catch (error) {
...
async function syncStudentData(studentId: string, handle: string) {
  ...
  try {
    ...
    for (const submission of acceptedSubmissions.slice(0, 100)) {
      ...
      const problemId = `${submission.problem.contestId || 'unknown'}-${submission.problem.index}`;
    ...
    for (const rating of ratingHistory.slice(0, 50)) {
      if (!rating.contestId || !rating.contestName || !rating.ratingUpdateTimeSeconds) {
        continue;
      }
      ...
      if (!existingContest) {
        const { error: insertError } = await supabase
          ...
          .insert({
            ...
            contest_date: new Date(rating.ratingUpdateTimeSeconds * 1000).toISOString(),
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Sync request received:', { studentId, codeforcesHandle, syncAll });
    ...
    if (syncAll) {
      ...
      for (const student of students) {
        try {
          const result = await syncStudentData(student.id, student.codeforces_handle);
          results.push({ 
      ...
      return new Response(JSON.stringify({ 
        ...
        message: 'Provide either (studentId, codeforcesHandle) or syncAll=true'
      }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
